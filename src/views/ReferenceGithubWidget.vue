...
export default {
	...
	computed: {
		...
		authorAndDateSubText() {
			if (this.isIssue) {
				...
				if (this.richObject.state === 'open') {
					...
					if (this.richObject.merged) {
						return t('integration_github', 'was merged on {relativeDate} by {creator}', {
		...
		createdAtSubText() {
			return t('integration_github', 'was opened on {relativeDate} by {creator}', {
				relativeDate: moment(this.richObject.created_at).fromNow(),
				creator: this.getUserLink(this.richObject.user?.login),
			}, null, { escape: false })
		},
		closedAtSubText() {
			return t('integration_github', 'was closed on {relativeDate} by {creator}', {
				relativeDate: moment(this.richObject.closed_at).fromNow(),
				creator: this.getUserLink(this.richObject.user?.login),
			}, null, { escape: false })
		},
